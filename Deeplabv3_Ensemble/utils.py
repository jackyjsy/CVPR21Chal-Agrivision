...
def new_intersectionAndUnion(pr, gt, threshold=None):
    """Calculate Intersection and Union between ground truth and prediction
    Args:
        pr (torch.Tensor): predicted tensor
        gt (torch.Tensor):  ground truth tensor
    ...
    pr = _threshold(pr, threshold=threshold)
    #multi_label_gt = torch.sum(gt,dim=1,keep_dim = True)
    multi_label_gt = _threshold(torch.sum(gt,dim=1,keepdim = True),threshold=1)*gt
    multi_label_pr = torch.sum(pr*multi_label_gt,dim=1,keepdim = True)*multi_label_gt - pr*multi_label_gt
    intersection = torch.sum(gt * pr+multi_label_pr, dim=[0, 2, 3])
    union = torch.sum(gt, dim=[0, 2, 3]) + torch.sum(pr, dim=[0, 2, 3]) - intersection
