...
class BaseDataset(torch.utils.data.Dataset):
    def __init__(self, odgt, opt, **kwargs):
        ...
        #self.classes = ['double_plant'  ,'drydown'  ,'endrow'  ,'nutrient_deficiency'  ,'planter_skip'  ,'storm_damage'  ,'water'  ,'waterway'  ,'weed_cluster']
        self.classes = ['background','double_plant'  ,'drydown'  ,'endrow'  ,'nutrient_deficiency'  ,'planter_skip'  ,'water'  ,'waterway'  ,'weed_cluster']
    ...
    def img_transform(self, rgb, nir):
        ...
        return img
    def img_transform_mscg(self, rgb, nir):
        # 0-255 to 0-1
        rgb = np.float32(np.array(rgb)) / 255.
        nir = np.expand_dims(np.float32(np.array(nir)), axis=2) / 255.

        rgb = torch.tensor(rgb.transpose((2, 0, 1)))
        nir = torch.tensor(nir.transpose((2, 0, 1)))

        #rgb = self.rgb_normalize(torch.from_numpy(rgb))
        #nir = self.nir_normalize(torch.from_numpy(nir))

        if self.channels == 'rgbn':
            img = torch.cat([nir,rgb], axis=0)
        elif self.channels == 'rgb':
            img = rgb
        elif self.channels == 'nir3':
            img = torch.cat([nir, nir, nir], axis=0)
...
class AgriTrainDataset(BaseDataset):
    ...
    def __getitem__(self, index):
        ...
        img_mscg = self.img_transform_mscg(rgb, nir)
        ...
        return img,img_mscg, valid_mask, label, info
...
class AgriTestDataset(BaseDataset):
    ...
    def __getitem__(self, index):
        ...
        return img,img_mscg, valid_mask, info
    ...
    def img_transform_mscg(self, rgb, nir):
        ...
        img = np.concatenate((np.expand_dims(nir, axis=2),rgb), axis=2) #shape as (512, 512, 4)
