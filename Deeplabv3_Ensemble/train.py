...
def main():
    ...
    if cfg.MODEL.arch == 'deeplab':
        ...
        model = smp.DeepLabV3(encoder_name='resnet101', classes=cfg.DATASET.num_class)
    elif cfg.MODEL.arch == 'FPN':
        model = smp.FPN(encoder_name='resnet101',classes=10)
    elif cfg.MODEL.arch == 'Unet':
        model = smp.Unet(encoder_name='resnet101',classes=10)
    elif cfg.MODEL.arch == 'Dinknet':
        if cfg.MODEL.backbone == 'resnet34':
            assert cfg.MODEL.ibn_mode == 'none'
            model = DinkNet34(num_classes=10)
            ...
            model = DinkNet50(num_classes=10, ibn_mode=cfg.MODEL.ibn_mode)
        elif cfg.MODEL.backbone == 'resnet101':
            model = DinkNet101(num_classes=10, ibn_mode=cfg.MODEL.ibn_mode)
...
def val(epoch, loader_val, model, loss_fn, history, args, logger):
    ...
    with torch.no_grad():
        for img, mask, label, _ in loader_val:
            ...
            intersection, union = intersectionAndUnion(pred1.data, label.data, 0.5)
